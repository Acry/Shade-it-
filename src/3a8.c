 ...
 * 
 * auto screenshot after 5 seconds from STFS
 * overwrite with key s
 * they are all the same size and saved in #define ST "assets/glsl/st/"
 * 
 ...
 * IMG_SaveJPG
 * 
 * intern function: save_screenshot(int overwrite)
 * 
 */
...
// 		SDL_Log("%s",argv[1]);
	...
	Window = SDL_CreateWindow("3a8 - screenshot",
...
// 	SDL_Log("%s",fullpath);
	...
	if( access( fullpath, F_OK ) != -1 ) {
		printf("Texture exists, nothing to do!\n");
		return 0;
	} else {
		printf("Texture does not exist, trying to download...\n");
		CURL *image; 
