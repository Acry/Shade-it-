...
#define EDIT_FILE "edit.stfs.glsl"
...
// 		SDL_Log("%s",argv[1]);
	...
	Window = SDL_CreateWindow("3b6 | edit current shader",
...
struct epoll_event reset_poll(int choice)
{
	
	static struct epoll_event ev;
	
	if (choice==0){// 0 activate NEW_FILE
		fd = inotify_init();
	...
	if (choice==1){// 1 deactivate NEW_FILE
		// release epoll
	...
	if (choice==2){// 2 activate EDIT_FILE
		fd = inotify_init();
		wd = inotify_add_watch(fd, ""NEW_FILE, IN_CLOSE_WRITE);
		ev.events = EPOLLIN|EPOLLOUT|EPOLLONESHOT;
		efd = epoll_create(sizeof(fd));
	...
	if (choice==3){// 3 deactivate EDIT_FILE
		// release epoll
		// NOTE One can't rely on close to clean up epoll subscriptions. 
		// NOTE One always must always explicitly call epoll_ctl(EPOLL_CTL_DEL)
		//      before calling close().
